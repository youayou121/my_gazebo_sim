...
class odom_class:
    ...
    def callback_join(self, data):
        ...
        #try:
        # 编码器 测速
        lrw_topic = data.name.index('rear_left2chasis')
        rrw_topic = data.name.index('rear_right2chasis')
        # 平均速度
        velocity = (data.velocity[lrw_topic] + data.velocity[rrw_topic])/(2 * 10.0)
        # 平均路程
